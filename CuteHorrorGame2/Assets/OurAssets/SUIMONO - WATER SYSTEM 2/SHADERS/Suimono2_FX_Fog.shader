...
CGINCLUDE
	...
	half4 ComputeFog (v2f i, bool distance, bool height) : SV_Target
	{
		...
		// Compute fog amount
		//half fogFac = saturate(max(0.0,g) * _SceneFogParams.z + _SceneFogParams.w);
		...
		half4 fCol = lerp (lerp(sceneColor,_FogColor*0.4646,_FogColor.a), sceneColor, fogFac);

		//lerp color to only depth value
		fCol.rgb = sceneColor.rgb;
		fCol.a = lerp (lerp(0.0,1.0,_FogColor.a), 0.0, fogFac);
