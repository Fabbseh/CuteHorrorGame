Shader "Suimono2/water_dx11" {

Properties {
//_suimono_TransTex("Transparent Tex", 2D) = "white" {}
	_OceanBlendTex("OceanBlendTex", 2D) = "white" {}
	_temp ("temp", Range(1.0, 1.5)) = 1.0
...
#pragma surface surf SuimonoDepth addshadow vertex:vertexSuimonoDisplaceDX11 tessellate:tessDistance
//#include "UnityPBSLighting.cginc"
//#pragma surface surf StandardSpecular addshadow vertex:vertexSuimonoDisplaceDX11 tessellate:tessDistance
...
inline fixed4 LightingSuimonoDepth (SurfaceOutput s, fixed3 lightDir, half3 viewDir, fixed atten)
{
	...
	//Edge Blend
	background = lerp(background,bgLayerTrans.rgb,edgeFactorDef);
	...
	//add edge blend
	//cP.rgb = lerp(cP.rgb,bgLayerTrans.rgb,depthMap.a*saturate((1.0-mask2)*2.0)*_EdgeBlend);
...
void surf (Input IN, inout SurfaceOutput o) {
	...
	//o.Normal = lerp(o.Normal,lerp(o.Normal,half3(0,1,0),0.7),mask1);
...
//edgeFactorDef = saturate(1.0-saturate(_EdgeBlend * (depth3-IN.screenPos.w)));
//if (_isForward == 1.0){
	edgeFactorDef = depthMap.a*saturate((1.0-mask2)*2.0)*_EdgeBlend;
//}
...
foamSpread = foamSpread + (waveColor.a*_ShallowFoamAmt)*(depthMap.r)*(_WaveShoreHeight)*_enableShoreline;
